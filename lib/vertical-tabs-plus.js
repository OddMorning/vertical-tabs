  ...
  general: {
    ...
    properties: {
      ...
      normalizeTooltips: {
        ...
        type:        'boolean',
        default:     false,
        description: 'Moves tab tooltips to side',
        order:       3,
      },
    },
  },
...
Common = {
  ...
  scrollTabs(ev) {
    ...
    // Old method uses 'pane:show-next-item' that doesn't always work as expected.
    // It tries to scroll other pane when it's focused.
    // New method is a hack but it works always as expected and doesn't depend on focused pane.
    // The old methos will be removed if the alt one will work without bugs.
    let oldMethod = false;
    ...
    if (oldMethod) {
      ...
      let
        activeTab = GUI.wrapper.querySelector('.tab.active'),
        tabDirection = ev.deltaY < 0,
        newTab = activeTab[tabDirection ? 'previousElementSibling' : 'nextElementSibling'];

      // Jump to the first or the last tab if needed
      if (!newTab || newTab.tagName != 'LI') {
        newTab = GUI.wrapper.querySelector(tabDirection ? '.tab:last-child' : '.tab:first-child');
      }
