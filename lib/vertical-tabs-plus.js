...
Controller = {
  ...
  tabsPlacement(optionID, oldOptionID) {
    ...
    if (value == 'over-tree') {
      ...
      // Нужно в инициализации, чтобы не мерцало, когда панелька есть
      // Нужно здесь, чтобы плагин не крэшился при создании нового окна
      // (в новом окне по умолчанию панелька скрыта)
      if (GUI.treeView) {
...
Common = {
  ...
  activate() {
    ...
    // Панелька с файлами проекта на случай, если нужно встроиться туда
  ...
  switchTabsByScrolling(ev) {
    // Предотвращает скролл, если включен скролл вкладок и если вкладки не вмещаются в экран
    ev.preventDefault();
...
Helper = {
  ...
  waitForProjectTreePanel(callback) {
    return new Promise((resolve, reject) => {
      // Заставляет панельку проекта появиться
      ...
      let
        ...
        waitingForElement = setInterval(() => {
          ...
          // Если исчерпалось количество попыток
          if (attemptCurrent == attemptCount) {
            clearInterval(waitingForElement);

            // Ставит расположение вкладок по умолчанию
            atom.config.set(
              'vertical-tabs-plus.tabsPlacement',
