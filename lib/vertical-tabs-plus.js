...
Controller = {
  ...
  'tabContainer.maxHeight'(value) {
    // let
    //   wrapperCount = Object.keys(GUI.wrappers).length,
    //   maxHeight = value / wrapperCount;
    // Helper.forEachWrapper(wrapper => {
    //   Helper.CSSvar(wrapper, 'max-tab-container-height', `${maxHeight}%`);
    // });
    Helper.forEachWrapper(wrapper => {
...
Common = {
  ...
  activate() {
    ...
    // Get current panes
    // Helper.forEachPane(pane => Common.onNewPane(pane, true));
