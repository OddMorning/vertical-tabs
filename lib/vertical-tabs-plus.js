  ...
  'general.tabsPlacement'(value, oldValue) {
    ...
    if (value == 'tree-view') {
      let moveTabsOverProjectTree = () => {
        ...
        GUI.treeViewProject.addEventListener('mouseenter', Common.onTreeViewProjectMouseMovement);
        GUI.treeViewProject.addEventListener('mouseleave', Common.onTreeViewProjectMouseMovement);
...
Common = {
  ...
  activate() {
    ...
    if (GUI.treeView)
      GUI.treeViewProject = GUI.treeView.querySelector('.tree-view-root');
  ...
  /* -------------------------------------------------------------------------------------------- */
  // Control mouse enter/leave on project tree when tabs are embedded
  onTreeViewProjectMouseMovement(ev) {
    const
      isMouseEnter = (ev.type == 'mouseenter'),
      onEnter = (wrapper) => {
        // If visible (offset) and full (scroll) heights are equal,
        // set fixed height to prevent project tree movement
        if (wrapper.offsetHeight == wrapper.scrollHeight) {
          wrapper.style.height = `${wrapper.offsetHeight}px`;
        }

        // If they don't match, there's no need in such manipulation
        // because tabs panel won't become taller in any case
        else {
          wrapper.dataset.noStretchNeeded = true;
        }
      },
      onLeave = (wrapper) => {
        // If full height is equal to the one set in `onEnter`,
        // no new tabs were opened so there's nothing to animate
        if (
          wrapper.style.height
          && wrapper.style.height.slice(0, -2) == wrapper.scrollHeight
        ) {
          // console.log('EQUAL', ev);
          wrapper.style.height = '';
        }

        // If `noStretchNeeded` is set, just remove it
        else if (wrapper.dataset.noStretchNeeded) {
          // console.log('NOSCRETCHNEEDED', ev);
          delete wrapper.dataset.noStretchNeeded;
          if (wrapper.style.height) wrapper.style.height = '';
        }

        // Check if mouse has been moved to nowhere.
        // It's needed because sometimes it happends itself.
        // It's some kind of mouseleave's (or windows') bug I guess
        else if (ev.toElement === null) {
          // console.log('ISNULL', ev);
          return;
        }

        // Set real height (CSS animation can animate between specific values only)
        // and set it back to dynamic after animation is finished
        else {
          // console.log('ELSE', ev);
    ...
    // Execute different functions due to event type
    Helper.forEachWrapper(wrapper => {
      if (isMouseEnter)
        onEnter(wrapper);
      else
  ...
  onWrapperClick(ev) {
    ...
    onDragStart = (tabEl) => {
      ...
      writeFixedPosition(GUI.treeViewProject);
      Helper.forEachWrapper(wrapper => {
      ...
      applyFixedPosition(GUI.treeViewProject);
      Helper.forEachPane((pane, id) => {
        let
    ...
    onDragEnd = () => {
      ...
      setTimeout(() => {
        ...
        unfixElement(GUI.treeViewProject);
        Helper.forEachWrapper(wrapper => {
